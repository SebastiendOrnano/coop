...
select 
    ...
    '/a_info/section_update_0.sql?section_id=' || (select section_id FROM  info_sections WHERE section_id = $section_id)  as action;
...
SELECT 
    'section_publication_status'        as name,
    'statut de publication actuel'      as label,
    'select'                            as type,
    4                    AS width,
    '1: en attente de publication / 2 : publié / archive' as description,
    (select section_publication_status FROM  info_sections WHERE section_id = $section_id)   as value,
