...
SET project_number = select count(*) from projects where project_status='active';
SET project_worksequences = select count(*) from project_worksequences where worksequence_status='active';
SET number_hours = select SUM(worksequence_duration_scheduled) from project_worksequences  where worksequence_status='active';
SET number_participants = select SUM(worksequence_nb_participants) from project_worksequences  where worksequence_status='active';
...
SET workpackage_id = select workpackage_id from project_workpackages where project_id=$project_id and workpackage_status='active';
SET worksequence_id = select worksequence_id from project_worksequences where workpackage_id=$workpackage_id and worksequence_status='active';
SET worksequence_nb_participants = select  worksequence_nb_participants from project_worksequences where worksequence_id=$worksequence_id  and worksequence_status='active';
...
select 
    ...
    $project_worksequences                as value,
    'blue'                                as value_color,
    4                                      as size,
