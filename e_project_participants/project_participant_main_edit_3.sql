...
SELECT 
    ...
    '/e_project_participants/project_participant_main_update_0.sql?project_participant_id='||$project_participant_id||'&project_id='||$project_id AS action;
...
select 
   'prm1_id' as name,
    ...
    WHERE project_participant_id=$project_participant_id) as value ;
...
select 
   'prm2_sub_id' as name,
    ...
    TRUE as readonly,
    6 AS width,
    (SELECT  c.prm2_sub_name FROM project_participants as p
...
SELECT 
    ...
    'select'                       as type,
    4                              AS width,
    (SELECT project_participant_role FROM project_participants WHERE project_participant_id=$project_participant_id) AS value,
...
SELECT 
    'project_participant_status'      as name,
    'Statut actif-archive'          as label,
    'select'                       as type,
    4                              AS width,
    (SELECT project_participant_status FROM project_participants WHERE project_participant_id=$project_participant_id) AS value,
