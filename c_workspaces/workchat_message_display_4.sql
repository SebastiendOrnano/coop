...
SELECT
    ...
    THEN (SELECT '[Edit](/c_workspaces/workchat_message_edit_4.sql?message_id='||message_id||'&workspace_id='||$workspace_id||'&topic_id='||$topic_id||') &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
[Delete](/c_workspaces/workchat_message_delete_0.sql?message_id='||message_id||'&workspace_id='||$workspace_id||'&topic_id='||$topic_id||')') 
    ELSE ''
    END AS View   
FROM forum_messages
...
SELECT
 ...
 SET  message_recipient_id = (SELECT topic_author_id FROM forum_topics where topic_id=$topic_id) ;
