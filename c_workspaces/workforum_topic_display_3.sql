...
WHEN (SELECT workspace_participant_role FROM workspace_participants WHERE  workspace_participant_id=$user_id AND workspace_id=$workspace_id) = 'coordinator' 
    THEN  ')&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[Close](/c_workspaces/workforum_topic_close_0.sql?topic_id='
    ELSE '' END ;
...
SELECT       -- NOTE: the dots join values from the forum_topics and forum_messages tables
    ...
    '[Discussion](/c_workspaces/workforum_message_display_3.sql?topic_id='||t.topic_id||'&workspace_id='||$workspace_id||$forum_close||t.topic_id||'&workspace_id='||$workspace_id||')'   AS View
