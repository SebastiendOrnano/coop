...
SELECT 
    ...
    'Clear' as reset,
    '/e_projects/project_main_update_0.sql?project_id='||$project_id  as action;
 ...
 SELECT 
    ...
    2 AS width,
    TRUE                     as required,
    (select project_status  FROM projects WHERE project_id = $project_id) as value,
...
SELECT
    ...
    'textarea' as type,  
    'Pitch' as label,
    (select project_summary  FROM projects WHERE project_id = $project_id) as value;

SELECT 
    'project_date_creation' as name,
    'Date de création'       as label,
    'date'                      as type,
    (select project_date_creation  FROM projects WHERE project_id = $project_id) as value,
    3                            AS width,
...
SELECT 
    ...
    'select'                       as type,
    6                              AS width,
    (select project_category  FROM projects WHERE project_id = $project_id) as value,
